...
export class PedigreeController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: '血統書データが正常に作成されました' })
  ...
  @ApiQuery({ name: 'limit', required: false, description: '1ページあたりの件数', example: 10 })
  ...
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '血統書データが見つかりません' })
  ...
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '血統書データが見つかりません' })
  @ApiParam({ name: 'id', description: '血統書データのID' })
  @ApiQuery({ name: 'generations', required: false, description: '取得する世代数', example: 3 })
  getFamily(@Param('id') id: string, @Query('generations') generations?: number) {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: '血統書データが正常に更新されました' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '血統書データが見つかりません' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: '無効なデータです' })
  @ApiParam({ name: 'id', description: '血統書データのID' })
  update(@Param('id') id: string, @Body() updatePedigreeDto: UpdatePedigreeDto) {
    return this.pedigreeService.update(id, updatePedigreeDto);
  }

  @Delete(':id')
  @ApiOperation({ summary: '血統書データを削除' })
  @ApiResponse({ status: HttpStatus.OK, description: '血統書データが正常に削除されました' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '血統書データが見つかりません' })
  @ApiParam({ name: 'id', description: '血統書データのID' })
